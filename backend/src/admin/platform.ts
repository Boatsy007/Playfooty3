...
import { fetchPage, parseResults, parseFixtures, parseLadder, type ResultRow, type FixtureRow, type GoalKickerRow } from '../football/url-ingest.js'
import { importGoalKickers } from '../football/goal-kickers.js'
...
router.post('/goal-kickers/import', async (req, res) => {
  ...
  const goalKickerImporterVersion = 'playwright-retry-v3'

  if (!sourceUrl && (!Array.isArray(b.rows) || b.rows.length === 0)) {
    return res.status(400).json({ error: 'Paste a PlayHQ goal kickers/statistics URL.', goalKickerImporterVersion })
  }

  const rows = Array.isArray(b.rows) ? b.rows.map(row => ({
    ...
    sourceUrl,
  })).filter(row => row.playerName && row.clubName && row.leagueName) as GoalKickerRow[] : undefined

  const result = await importGoalKickers({ sourceUrl, rows, persistSource: true })
  if (result.data.imported === 0 && result.data.strategy === 'none') {
    return res.status(422).json({ error: 'No goal kicker rows could be parsed from this PlayHQ page.', goalKickerImporterVersion, data: { ...result.data, goalKickerImporterVersion } })
  }
  res.json({ goalKickerImporterVersion, data: { ...result.data, goalKickerImporterVersion }, errors: result.errors.slice(0, 20) })
})
