...
function GoalKickersCard({ rows, loading }: { rows: GoalKickerLeader[]; loading: boolean }) {
  return <article className="side-card goal-kickers-card"><CardTitle title="Top Goal Kickers" to="/goal-kickers" />
    {loading && <p className="empty-copy">Loading goal kickers…</p>}
    {!loading && rows.length === 0 && <p className="empty-copy">No goal kickers imported yet.</p>}
