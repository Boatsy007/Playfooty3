  ...
  return (
    <nav className="league-rail" aria-label="Active league rail">
      ...
      <div className="rail-scroll">
        ...
        {!loading && leagues.slice(0, 24).map(l => (
          <Link key={l.id} to={leaguePath(l.id)} className="league-tile">
            <LeagueMark league={l} />
        ...
        {!loading && <Link to="/leagues" className="league-all">All leagues <ArrowRight size={14} /></Link>}
      </div>
    </nav>
  )
}
...
function HomepageStyles() { return <style>{`
  ...
  @media (max-width:820px){.front-page{padding:14px 12px 22px}.front-grid{display:flex;flex-direction:column}.editorial-column{order:1}.rankings-column{order:2;width:100%}.broadcast-sidebar{order:3;display:flex;width:100%}.broadcast-sidebar .side-card:nth-of-type(4),.broadcast-sidebar .side-card:nth-of-type(6){display:none}.mobile-extras{display:block}.secondary-grid{grid-template-columns:1fr}.secondary-story,.secondary-story:first-child{grid-template-columns:112px minmax(0,1fr)}.rank-row{grid-template-columns:28px 38px minmax(0,1fr) 48px 54px}.rank-row .form{display:none}.board-labels{grid-template-columns:1fr 58px}.board-labels span:nth-child(2){display:none}.league-tile{min-width:246px}.feature-copy{padding:20px}.feature-copy h2{font-size:3rem}.feature-copy p{font-size:16px}.feature-bottom{align-items:flex-start;flex-direction:column}.rail-live{display:none}}
  @media (max-width:520px){.rank-row{grid-template-columns:26px 34px minmax(0,1fr) 42px}.rank-row .movement{display:none}.ranking-board .rank-row:nth-of-type(n+18){display:none}.secondary-story,.secondary-story:first-child{grid-template-columns:96px minmax(0,1fr)}.broadcast-sidebar .side-card:nth-of-type(3){display:none}}
`}</style> }
