...
function RankingsStyles() { return <style>{`
  ...
  @media (max-width:760px){.rankings-page{padding:14px 12px 38px}.rankings-header{padding:18px;border-radius:16px}.header-copy h1{font-size:3.2rem}.header-copy p{font-size:15px}.filters-panel{grid-template-columns:1fr}.table-wrap{display:none}.ranking-cards{display:grid;gap:10px;padding:12px}.rank-card{display:grid;grid-template-columns:34px 44px minmax(0,1fr) auto;gap:10px;align-items:center;border:1px solid ${LINE};border-radius:14px;padding:12px;text-decoration:none;color:${TEXT};background:#fff;box-shadow:0 8px 20px rgba(6,42,95,.06)}.card-club{min-width:0}.card-club b,.card-club small{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.card-club small{color:${MUTED};font-size:11px}.card-side{display:grid;justify-items:end;gap:5px}.card-side strong{font-size:20px;color:${PINK}}.rankings-sidebar{display:flex}.rankings-sidebar .side-card:nth-of-type(2){order:3}.rankings-sidebar .side-card:nth-of-type(3){order:4}.rankings-sidebar .side-card:nth-of-type(4){order:2}.rankings-sidebar .side-card:nth-of-type(5){order:5}.ladder-toolbar a{display:none}}
`}</style> }
