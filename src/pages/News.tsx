...
import {
  loadPublished, allArticles, featuredArticles, latestArticles, trendingArticles, articlesInCategory, searchArticles,
  ...
  return (
    <div style={{ background: '#fff', minHeight: '100vh', color: TEXT }}>
      ...
      <main className="news-page">
        ...
        {hero ? <FeaturedStory article={hero} /> : <NewsEmptyState />}
        <div className="news-layout">
          <section className="news-main" aria-label="Latest stories">
            <SectionTitle title="Latest Stories" />
            {latest.length > 0 ? <div className="story-grid">{latest.map(a => <StoryCard key={a.slug} article={a} />)}</div> : <p className="empty-copy">No additional published stories yet.</p>}
            <TrendingSection articles={trending} />
            <NewsBrowse categories={categories} />
...
function PageHeader({ total, updated, categories }: { total: number; updated: string | null; categories: CategoryId[] }) {
  return <header className="news-header">
    ...
    {categories.length > 0 && <nav className="category-nav" aria-label="News categories">{categories.map(id => <a key={id} href={`#cat-${id}`}>{categoryOf(id).label}</a>)}</nav>}
  </header>
}
...
function ChampionshipTeaser() { return <Link to="/championship" className="side-card championship"><CalendarDays /><span>Future events</span><strong>Future showcase concept</strong><small>Coming soon</small></Link> }
...
function NewsEmptyState() { return <section className="sports-empty"><h2>No published articles yet</h2><p>Country football news will appear here once articles are published.</p></section> }

function NewsBrowse({ categories }: { categories: CategoryId[] }) {
  ...
  return <section className="news-search" aria-label="Search news">
    ...
    <div className="filter-row">
      {categories.length > 0 && <Filter value={f.category ?? ''} onChange={v => set('category', v)} placeholder="All Categories" options={categories.map(id => ({ id, name: categoryOf(id).label }))} />}
