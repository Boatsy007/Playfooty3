...
function seasonYear(d: LeagueDetail): string {
  return d.currentSeason?.match(/\d{4}/)?.[0] ?? String(new Date().getFullYear())
}

function HeroSkeleton() {
  return (
    <div style={{ background: '#0c0e13', padding: '52px 20px 46px' }}>
      <div style={{ maxWidth: 1120, margin: '0 auto' }}>
        <Skel w={200} h={12} style={{ marginBottom: 30, background: 'rgba(255,255,255,0.08)' }} />
        <Skel w="55%" h={64} style={{ marginBottom: 18, background: 'rgba(255,255,255,0.1)' }} />
        <Skel w={280} h={14} style={{ background: 'rgba(255,255,255,0.08)' }} />
        <span className="font-condensed" style={{ display: 'block', marginTop: 22, color: MUTE, fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase' }}>Loading league</span>
      </div>
    </div>
  )
}
...
function buildJsonLd(d: LeagueDetail, facts: NonNullable<ReturnType<typeof deriveFacts>>, leagueId: string) {
  ...
  if (facts.bestClub) {
    faqs.push({
      q: `Which ${d.name} club is ranked highest in Australia?`,
      a: `${facts.bestClub.clubName} is the highest nationally ranked club in the ${d.name}, at #${facts.bestClub.rank} in Australia with a power rating of ${facts.bestClub.powerRating.toFixed(1)}.`,
    })
  }
  const story = weeklyStory(d, facts)
  ...
  return [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: 'Leagues', item: `${base}/leagues` },
